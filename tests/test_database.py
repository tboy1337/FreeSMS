#!/usr/bin/env python3
"""
Test script for FreeSMS database module with comprehensive coverage
"""
...
class TestDatabaseComprehensive:
    ...
    def test_default_database_path(self):
        ...
        mock_app_dir = mock_home / ".freesms"
        mock_db_path = mock_app_dir / "freesms.db"
    ...
    def test_api_credentials_error_handling(self):
        ...
        with patch.object(
            self.db, "_init_db", return_value=True
    ...
    def test_scheduled_message_with_recurrence_data(self):
        ...
        recurrence_data = {"interval": 2, "unit": "weeks", "end_date": "2024-12-31"}
    ...
    def test_update_scheduled_message(self):
        ...
        result = self.db.update_scheduled_message(
            ...
            scheduled_time=datetime.strptime(
                "2024-01-15 15:30:00", "%Y-%m-%d %H:%M:%S"
            ),
            service="textbelt",
            recurring="weekly",
    ...
    def test_update_scheduled_message_not_found(self):
        ...
        result = self.db.update_scheduled_message(
            message_id=9999, message="Updated message"
        )
    ...
    def test_update_scheduled_message_error(self):
        ...
        with patch.object(self.db, "_init_db", return_value=True):
            ...
            try:
                ...
                result = self.db.update_scheduled_message(
                    message_id=message_id, message="Updated message"
                )
                assert not result
            finally:
                # Restore original connection
    ...
    def test_message_templates(self):
        ...
        result = self.db.save_message_template(
            "Test Template", "Hello {name}, this is a test template."
        )
        assert result
    ...
    def test_save_api_credentials_update_existing(self):
        ...
        assert self.db.save_api_credentials(
            service_name, initial_creds, is_active=False
        )
    ...
    def test_update_scheduled_message_individual_fields(self):
        ...
        result = self.db.update_scheduled_message(
            message_id=message_id, scheduled_time=new_time
        )
        assert result
        ...
        result = self.db.update_scheduled_message(
            message_id=message_id, scheduled_time="2025-12-25 15:30:00"
        )
        assert result

    def test_get_api_credentials_legacy_migration(self):
        """Legacy plaintext credentials are migrated to encrypted storage on read."""
        plaintext = json.dumps(
            {
                "account_sid": "AClegacy",
                "auth_token": "tokenlegacy",
    ...
    def test_get_api_credentials_unknown_format(self):
        """Unknown credential format returns None."""
        cursor = self.db.conn.cursor()
        cursor.execute(
            "INSERT INTO api_credentials (service_name, credentials, is_active) "
            "VALUES (?, ?, ?)",
            ("twilio", "not-json-or-encrypted", 1),
        )
        self.db.conn.commit()
    ...
    def test_count_successful_sends_today(self):
        ...
        assert self.db.count_successful_sends_today("twilio") == 1

    def test_count_successful_sends_today_db_error(self):
