#!/usr/bin/env python3
"""
Test script for FreeSMS service manager
"""
...
class TestSMSServiceManager:
    ...
    def setup_method(self):
        ...
        self.db.get_api_credentials.side_effect = lambda service_name: {
            ...
            "textbelt": {"api_key": "test_key"},
        }.get(service_name)

        # Set up active services
        self.db.get_active_services.return_value = ["twilio"]
        self.db.count_successful_sends_today.return_value = 0
        ...
        self.mock_twilio_service.daily_limit = 100
        ...
        self.mock_textbelt_service.daily_limit = 1
        ...
        self.mock_textbelt_module.TextBeltService.return_value = (
            self.mock_textbelt_service
        )
    ...
    def test_set_active_service(self):
        ...
        self.db.save_api_credentials.assert_called_with(
            "textbelt", {"api_key": "test_key"}, is_active=True
        )
    ...
    def test_send_sms_with_active_service(self):
        ...
        self.mock_twilio_service.send_sms.assert_called_once_with(
            "+12345678901", "Test message"
        )
    ...
    def test_send_sms_with_specific_service(self):
        ...
        response = self.manager.send_sms(
            "+12345678901", "Test message", service_name="textbelt"
        )

        # Verify service was called
        self.mock_textbelt_service.send_sms.assert_called_once_with(
            "+12345678901", "Test message"
        )
    ...
    def test_send_sms_rate_limit_enforced(self):
        """Test that daily send limit blocks additional messages."""
        self.mock_twilio_service.daily_limit = 2
        self.mock_twilio_service.service_name = "Twilio"
        self.db.count_successful_sends_today.return_value = 2

        response = self.manager.send_sms("+12345678901", "Test message")

        assert not response.success
        assert "Daily send limit reached" in response.error
        self.mock_twilio_service.send_sms.assert_not_called()

    def test_send_sms_invalid_daily_limit_type(self):
        """Non-numeric daily limits are treated as zero (no blocking)."""
        self.mock_twilio_service.daily_limit = "invalid"
        self.mock_twilio_service.service_name = "Twilio"
        self.db.count_successful_sends_today.return_value = 999

        response = self.manager.send_sms("+12345678901", "Test message")

        assert response.success
        self.mock_twilio_service.send_sms.assert_called_once()
    ...
    def test_check_delivery_status_with_exception(self):
        ...
        # Make the service raise an exception
        self.mock_twilio_service.get_delivery_status.side_effect = Exception(
            "Test exception"
        )
