...
class TestMain:
    ...
    def test_main_gui_mode_basic_components(self):
        ...
        notification = NotificationService("Test FreeSMS")
    ...
    def test_main_gui_mode_argument_flow(self):
        ...
        test_cases = [
            ...
            (
                ["main.py", "--minimized"],
                {"minimized": True, "debug": False, "cli": False},
            ),
    ...
    def test_main_gui_mode_icon_path_logic(self):
        ...
        icon_path = os.path.join(
            os.path.dirname(main_file), "gui", "assets", "sms_icon.png"
        )
...
class TestModuleImport:
    ...
    def test_project_root_path_insertion(self):
        ...
        # Calculate the expected project root (FreeSMS directory)
        current_file_dir = os.path.dirname(os.path.abspath(__file__))  # tests dir
        expected_project_root = os.path.dirname(current_file_dir)  # FreeSMS dir
        ...
        ), "Project root should be inserted at the beginning of sys.path"
...
class TestMainExecution:
    ...
    def setup_method(self):
        """Set up patches and argv for main() execution tests."""
        self.original_argv = sys.argv.copy()
        self.migrate_patcher = patch("src.main.migrate_legacy_data")
        self.log_dir_patcher = patch("src.main.get_log_dir")
        self.mock_migrate = self.migrate_patcher.start()
        self.mock_log_dir = self.log_dir_patcher.start()
        self.mock_log_dir.return_value = Path(tempfile.mkdtemp())

    def teardown_method(self):
        """Restore argv and stop patches."""
        sys.argv = self.original_argv
    ...
    def test_main_name_execution(self, mock_main):
        ...
        exec(
            "if True: import src.main; src.main.main()"
    ...
    @patch("src.main.setup_logger")
    def test_main_gui_mode_full_execution(
        self,
        mock_logger,
        mock_qicon,
        mock_qapp,
        ...
        assert logger_args[0] == "freesms"
        assert logger_args[1] == logging.INFO
        assert logger_kwargs["log_file"].endswith(".log")
        mock_config_service.assert_called_once_with("freesms", config_path=None)
        mock_notification_service.assert_called_once_with("FreeSMS")
        ...
        mock_sms_app.assert_called_once_with(
            config=mock_config, notification=mock_notification
        )
    ...
    @patch("src.main.QApplication")
    @patch("src.main.setup_logger")
    def test_main_gui_mode_minimized(
        self,
        mock_logger,
        mock_qapp,
        mock_config_service,
        ...
        mock_notification.send_notification.assert_called_once_with(
            "FreeSMS", "Application started and running in background"
        )
    ...
    def test_main_gui_mode_debug(
        ...
        mock_logger.assert_called_once()
        logger_args, _ = mock_logger.call_args
        assert logger_args[0] == "freesms"
        assert logger_args[1] == logging.DEBUG
    ...
    @patch("src.main.QApplication")
    @patch("src.main.setup_logger")
    def test_main_gui_mode_with_icon(
        self,
        mock_logger,
        mock_qapp,
        mock_config_service,
    ...
    @patch("src.main.QApplication")
    @patch("src.main.setup_logger")
    def test_main_gui_mode_icon_missing(
        self,
        mock_logger,
        mock_qapp,
        mock_config_service,
    ...
    @patch("src.main.QApplication")
    @patch("src.main.setup_logger")
    def test_main_gui_mode_icon_exception(
        self,
        mock_logger,
        mock_qapp,
        mock_config_service,
        ...
        # Mock QIcon to raise a handled runtime error
        with patch("PySide6.QtGui.QIcon", side_effect=RuntimeError("Icon error")):
            # Run main function
            main()

            # Verify warning was logged
            mock_logger_instance.warning.assert_called_once()
            warning_args, _ = mock_logger_instance.warning.call_args
    ...
    @patch("src.main.QApplication")
    @patch("src.main.setup_logger")
    def test_main_gui_mode_config_minimized_setting(
        self,
        mock_logger,
        mock_qapp,
        mock_config_service,
