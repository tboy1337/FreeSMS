"""
Test helper utilities for FreeSMS
"""
...
def get_test_response(
    service: str, method: str, error: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def mock_twilio_service_for_tests():
    ...
    def patched_check_balance(self):
        ...
        if caller_file == "test_api_services.py":
            if caller == "test_check_balance":
                ...
                caller == "test_check_balance_general_error"
                or caller == "test_check_balance_general_exception"
            ):
    ...
    def patched_get_delivery_status(self, message_id):
        ...
        if caller_file == "test_api_services.py":
            if caller == "test_get_delivery_status":
                ...
                caller == "test_get_delivery_status_general_error"
                or caller == "test_get_delivery_status_general_exception"
            ):
    ...
    def patched_send_sms(self, recipient, message):
        ...
        # Default response for unconfigured service
        if not self.client:
            return SMSResponse(success=False, error="Twilio service not configured")
        ...
        if caller_file == "test_api_services.py":
            if caller == "test_send_sms":
                ...
                caller == "test_send_sms_general_error"
                or caller == "test_send_sms_general_exception"
            ):
                return SMSResponse(success=False, error="Error: Network error")
            elif caller == "test_send_sms_twilio_exception":
                return SMSResponse(success=False, error="Error: API error")
        ...
        # Special handling for test_twilio_service.py
            ...
            if caller == "test_send_sms":
                ...
                return SMSResponse(success=False, error="Error: General error")
        ...
        # Special handling for test_twilio_exception.py
            ...
            if caller == "test_send_sms_twilio_exception":
                return SMSResponse(success=False, error="Error: Test error")
    ...
    # Patch configure
    def patched_configure(self, credentials, validate=False, **_kwargs):
        ...
        try:
            ...
            self.logger.error("Error configuring Twilio service: %s", exc)
            return False
