...
class TestTwilioServiceDetailed:
    ...
    def setup_method(self):
        ...
        self.module_exception_patcher = patch(
            "src.api.twilio_service.TwilioRestException"
        )
    ...
    def test_configure_exception(self):
        ...
        with patch(
            "src.api.twilio_service.Client", side_effect=Exception("Connection error")
        ):
            result = self.service.configure(self.credentials)
            assert not result
    ...
    def test_load_env_credentials(self):
        ...
        with patch("os.environ.get", side_effect=lambda key: env_values.get(key)):
            ...
            with patch.object(
                TwilioService, "configure", return_value=True
            ) as mock_configure:
                # Create service
    ...
    def test_validate_credentials(self):
        ...
        with patch.object(
            self.service, "validate_credentials", return_value=True
        ) as mock_validate:
    ...
    def test_check_balance_api_error(self):
        ...
        with patch.object(
            self.service, "check_balance", return_value={"error": "API error"}
        ):
            # Check balance
            balance = self.service.check_balance()
    ...
    def test_check_balance_general_error(self):
        ...
        # Patch check_balance to return an error response
        with patch.object(
            self.service, "check_balance", return_value={"error": "Network error"}
        ):
            # Check balance
            balance = self.service.check_balance()
    ...
    def test_get_delivery_status_general_error(self):
        ...
        with patch.object(
            self.service,
            "get_delivery_status",
            return_value={"status": "error", "error": "Network error"},
        ):
            # Get status
            status = self.service.get_delivery_status("SM123")
...
class TestTwilioServiceImplementation:
    """Execute TwilioService methods without patching the methods under test."""

    def setup_method(self):
        """Set up mocked Twilio client on the module import path."""
        self.client_patcher = patch("src.api.twilio_service.Client")
        self.mock_client_class = self.client_patcher.start()
        self.mock_client = MagicMock()
        self.mock_client_class.return_value = self.mock_client
    ...
    def test_send_sms_success(self):
        """Send SMS through the real send_sms implementation."""
    ...
    def test_send_sms_twilio_style_error(self):
        """Map Twilio-style API errors in send_sms."""
        api_error = Exception("API error")
        api_error.msg = "Invalid number"
        api_error.code = 21211
        ...
        assert not response.success
        assert "Twilio API error" in response.error
        assert response.details["code"] == 21211

    def test_send_sms_general_error(self):
        """Map generic exceptions in send_sms."""
        self.mock_client.messages.create.side_effect = RuntimeError("network down")
    ...
    def test_check_balance_success(self):
        """Check balance through the real check_balance implementation."""
        account_side = MagicMock()
        account_side.fetch.return_value = MockAccount()
        account_side.balance.fetch.return_value = MockBalance()
    ...
    def test_check_balance_twilio_style_error(self):
        """Map Twilio-style API errors in check_balance."""
        api_error = Exception("auth failed")
        api_error.msg = "auth failed"
        api_error.code = 20003
        account_side = MagicMock()
        account_side.fetch.side_effect = api_error
        self.mock_client.api.accounts.return_value = account_side

        balance = self.service.check_balance()

        assert balance["error"] == "Twilio API error: auth failed"

    def test_get_delivery_status_success(self):
        """Fetch delivery status through the real implementation."""
        delivered_message = MockMessage()
        delivered_message.status = "delivered"
        messages_side = MagicMock()
        messages_side.fetch.return_value = delivered_message
    ...
    def test_get_delivery_status_general_error(self):
        """Map generic exceptions in get_delivery_status."""
        messages_side = MagicMock()
        messages_side.fetch.side_effect = RuntimeError("lookup failed")
        self.mock_client.messages.return_value = messages_side

        status = self.service.get_delivery_status("SM123")

        assert status["status"] == "error"
        assert "lookup failed" in status["error"]

    def test_validate_credentials_failure(self):
        """Return False when credential validation fails."""
        account_side = MagicMock()
        account_side.fetch.side_effect = RuntimeError("invalid creds")
        self.mock_client.api.accounts.return_value = account_side

        assert not self.service.validate_credentials()

    def test_validate_credentials_twilio_style_error(self):
        """Log Twilio-style errors during credential validation."""
        api_error = Exception("auth failed")
        api_error.msg = "auth failed"
        api_error.code = 20003
        account_side = MagicMock()
