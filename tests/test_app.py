#!/usr/bin/env python3
"""
Comprehensive pytest-qt GUI tests for FreeSMS application
"""
...
class TestSMSAppCore:
    ...
    def test_contact_manager(self):
        ...
        with patch(
            "src.models.contact_manager.ContactManager._validate_phone_number",
    ...
    def test_sms_response(self):
        ...
        success = SMSResponse(
            success=True, message_id="msg123", details={"status": "sent"}
        )
        ...
        error = SMSResponse(
            success=False, error="Connection failed", details={"code": 500}
        )
...
class TestSMSApplicationGUI:
    ...
    def test_app_initialization(
        ...
        with (
            patch("PySide6.QtWidgets.QMessageBox.question") as mock_msg,
            ...
            app = SMSApplication(
                config=setup["config"], notification=setup["notification"]
            )
            qtbot.addWidget(app)

            # Test window properties
            assert app.windowTitle() == "FreeSMS"
    ...
    def test_settings_tab(self, qtbot, setup_gui_test):
        ...
        mock_app.db = setup["db"]
        mock_app.service_manager = MagicMock()
        mock_app.service_manager.get_available_services.return_value = [
            "twilio",
            "textbelt",
        ]
        ...
        assert (
            tab.tab_widget.count() >= 2
        )  # Should have at least SMS Services and General
