...
class MessageScheduler:
    ...
    def _process_scheduled_message(self, message):
        ...
        try:
            ...
            if not message or not isinstance(message, dict):
                logger.warning("Invalid message object: %s", message)
                return False
            ...
            response = self.service_manager.send_sms(
                recipient=recipient, message=message_text, service_name=service
            )
    ...
    def _update_recurring_message(self, message):
        ...
        schedule_time = datetime.strptime(
            message["scheduled_time"], "%Y-%m-%d %H:%M:%S"
        )
        recurrence_data = message.get("recurring_interval")
        ...
        if next_schedule_time:
            ...
            self._trigger_callback(
                "message_rescheduled",
                {"message_id": message_id, "next_schedule": next_schedule_str},
            )
    ...
    def update_scheduled_message(
        ...
        with self.lock:
            ...
            if result:
                self._trigger_callback("message_updated", {"message_id": message_id})
        ...
        with self.lock:
            ...
            for message in messages:
                if "recurring_interval" in message and message["recurring_interval"]:
                    try:
                        message["recurring_interval"] = json.loads(
