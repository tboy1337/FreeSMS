...
class SMSApplication(QMainWindow):
    ...
    def __init__(self, config=None, notification=None):
        ...
        try:
            icon_path = os.path.join(
                os.path.dirname(__file__), "assets", "sms_icon.png"
            )
            if os.path.exists(icon_path):
                self.setWindowIcon(QIcon(icon_path))
        except (OSError, RuntimeError) as exc:
            logger.debug("Application icon not set: %s", exc)
    ...
    def _initialize_services(self):
        ...
        self.scheduler.register_callback(
            "message_sent", self._on_scheduled_message_sent
        )
        self.scheduler.register_callback(
            "message_failed", self._on_scheduled_message_failed
        )
    ...
    def _start_background_tasks(self):
        ...
        self.status_update_thread = threading.Thread(
            target=self._update_status_periodically
        )
        ...
        try:
            ...
            icon_path = os.path.join(
                os.path.dirname(__file__), "assets", "sms_icon.ico"
            )
    ...
    def send_message(self, recipient, message, service_name=None):
        ...
        threading.Thread(
            target=self._send_message_thread, args=(recipient, message, service_name)
        ).start()
    ...
    def _handle_send_response(self, response, recipient):
        ...
        if response.success:
            ...
            QMessageBox.information(
                self, "Success", f"Message sent successfully to {recipient}"
            )
            ...
            QMessageBox.critical(
                self, "Error", f"Failed to send message: {response.error}"
            )
