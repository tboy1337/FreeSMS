...
class SettingsTab(QWidget):
    ...
    def _load_current_settings(self):
        ...
        try:
            if (
                hasattr(self.app, "service_manager")
                and self.app.service_manager.active_service
            ):
    ...
    def _create_textbelt_form(self):
        ...
        self.use_free_tier_check.setChecked(
            credentials.get("api_key", "") == "textbelt"
        )
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", "Twilio credentials saved successfully"
                )
                self._load_current_settings()
            else:
    ...
    def _on_save_textbelt(self):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", "TextBelt credentials saved successfully"
                )
                self._load_current_settings()
            else:
                QMessageBox.critical(
    ...
    def _on_set_active_service(self):
        ...
        try:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", f"{service_name} set as active service"
                )
                self._load_current_settings()
            else:
                QMessageBox.critical(
